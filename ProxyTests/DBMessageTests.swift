...
class DBMessageTests: FirebaseTest {
    ...
    func testDeleteUnreadMessage() {
        ...
        FirebaseTest.sendMessage { (message, _, _, _) in
            FirebaseHelper.deleteUnreadMessage(message) { (success) in
                XCTAssert(success)
                let work = GroupWork()
    ...
    func testRead() {
        ...
        FirebaseTest.sendMessage { (message, _, _, receiver) in
            let date = Date()
            FirebaseHelper.read(message, date: date) { (success) in
                XCTAssert(success)
                let work = GroupWork()
                work.checkDeleted(Child.userInfo, message.receiverId, Child.unreadMessages, message.messageId)
    ...
    func testReadWithOtherUnreadMessages() {
        ...
        FirebaseTest.sendMessage { (message, _, _, _) in
            FirebaseTest.sendMessage { (_, _, _, receiver) in
                FirebaseHelper.read(message) { (success) in
                    XCTAssert(success)
                    let work = GroupWork()
    ...
    func testSendMessageWithSenderConvo() {
        ...
        FirebaseTest.sendMessage { (_, convo, _, _) in
            FirebaseHelper.sendMessage(convo: convo, text: FirebaseTest.text) { (result) in
...
//            FirebaseHelper.deleteProxy(receiverProxy) { (success) in
//                XCTAssert(success)
//                FH.getConvo(uid: senderConvo.senderId, key: senderConvo.key) { (convo) in
//                    guard let updatedConvo = convo else {
//                        XCTFail()
//                        return
//                    }
//                    FH.sendMessage(convo: updatedConvo, text: DBTest.text) { (result) in
//                        switch result {
