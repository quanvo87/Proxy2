...
class SenderProxyInfoCell: UITableViewCell {
    ...
    func observeIcon() {
        iconRef = ref.child(Path.Proxies).child(proxy.ownerId).child(proxy.key).child(Path.Icon)
    ...
    func observeNickname() {
        nicknameRef = ref.child(Path.Proxies).child(proxy.ownerId).child(proxy.key).child(Path.Nickname)
